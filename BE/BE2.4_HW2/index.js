...
async function deleteHotelById(hotelId){
  try {
    ...
    if(hotel){
     console.log('hotel successfully deleted by Id',hotel)

    }
    else{
      console.log('hotel not found by this ID')

    }

  } catch (error) {
    console.error('error to delete hotel by ID',error)
  }
}


// deleteHotelById("66dee9e3019daf9e9edccde4")


async function deleteHotelByPhoneNumber(phoneNumber){
  try {
    const hotel = await Hotel.findOneAndDelete({phoneNumber:phoneNumber});

    if(hotel){
      console.log('hotel deleted by phone number successfully, ',hotel)
    }
    else{
      console.log('hotel not found by this phone number')
    }
  } catch (error) {
    console.error('getting error to delete hotel by phone number',error)
  }
}

deleteHotelByPhoneNumber("+9876543210")