...
app.delete('/restaurants/:restaurantId', async (req, res) => {
  ...
  try {
    const restaurant = await deleteRestaurantById(restaurantId);

    if(restaurant){
      res.status(200).json({message:'restaurant deleted successfully',deleted:restaurant})
    }
    else{
      res.status(404).json({
    ...
    res.status(500).json({
      error:'failed to deleted restaurant'
    })
  }
})


// find restaurant by id and updated it

async function updateRestaurantById(restaurantId , dataToUpdate){
  try {
    const restaurant = await Restaurant.findByIdAndUpdate(restaurantId, dataToUpdate, {new:true});
    return restaurant
  } catch (error) {
    throw error
  }
...
app.post('/restaurants/:restaurantId', async (req, res) => {
  ...
  try {
    ...
    if(restaurant){
      res.status(200).json({
        message:'restaurant udpated successfully',
        updated:restaurant
      })
    }
    else{
