...
// find restaurant by id and delete it from the database;
...
async function deleteRestaurantById(restaurantId){
  try {
    ...
    if(!restaurant) {
      console.log('not found any restaurant related to restaurantId')
    }

    console.log('restaurant successfully delete by Id: ',restaurant)

  } catch (error) {
    console.error('getting error while deleting restaurant ',error)
  }
}


// deleteRestaurantById("66dedeecad34278f28bcfb92")


// find the restaurant by name and delete it from the database;
...
async function deleteRestaurantByName(restaurantName){
  try {
    ...
    const restaurant = await Restaurant.findOneAndDelete({name:restaurantName});

    if(!restaurant) {
      console.log('restaurant not found ')
    }
    console.log('restaurant successfully delete by name: ',restaurant)
...
// deleteRestaurantByName("Bella Italia")